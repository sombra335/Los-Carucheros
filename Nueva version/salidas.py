...
SALIDAS_HTML = os.path.join(BASE_DIR, "salidas.html")
...
def create_new():
    ...
    title = simpledialog.askstring("Título", "Introduce el título de la salida:")
    if not title: return
    subtitle = simpledialog.askstring("Subtítulo", "Introduce el subtítulo de la salida:")
    if subtitle is None: return
    text = simpledialog.askstring("Texto", "Introduce el texto de la salida:")
    ...
    # 4. Create salida folder structure
    slug = title.lower().replace(" ", "_")[:30]
    timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
    salida_folder = os.path.join(SALIDAS_DIR, f"{slug}_{timestamp}")
    images_folder = os.path.join(salida_folder, "images")
    os.makedirs(images_folder, exist_ok=True)

    # 5. Copy main image and gallery images
    main_img_filename = copy_image(main_img_path, salida_folder)
    gallery_img_filenames = []
    for img_path in gallery_img_paths:
        gallery_img_filenames.append(copy_image(img_path, images_folder))

    # 6. Create salida.html in salida_folder
    salida_filename = "salida.html"
    salida_filepath = os.path.join(salida_folder, salida_filename)
    ...
    with open(salida_filepath, "w", encoding="utf-8") as f:
...
<body>
    ...
    <div id="Galeria">
        ...
        <div class="container" style="text-align: center;">
            <img src="{first_gallery_img}" alt="imagen" class="gallery-img" id="gallery-img">
            <div style="margin-top: 10px;">
                <a id="download-btn" href="{first_gallery_img}" download>
                    <button
...
""")
    ...
    # 7. Insert new card at top of salidas.html
    with open(SALIDAS_HTML, "r", encoding="utf-8") as f:
        html = f.read()
    insert_pos = html.find('<div class="news">')
    if insert_pos == -1:
        messagebox.showerror("Error", "No se encontró el contenedor de salidas en salidas.html")
        return
    salida_rel_path = os.path.relpath(salida_filepath, BASE_DIR).replace("\\", "/")
    main_img_card_rel = os.path.relpath(os.path.join(salida_folder, main_img_filename), BASE_DIR).replace("\\", "/")
    card_html = f'''
        <a href="{salida_rel_path}"><div class="card" >
    ...
    with open(SALIDAS_HTML, "w", encoding="utf-8") as f:
        f.write(new_html)

    messagebox.showinfo("Éxito", f"Salida creada en: {salida_rel_path}")

root = tk.Tk()
root.title("Gestor de Salidas")
root.geometry("300x150")
btn = tk.Button(root, text="Crear Nueva Salida", command=create_new, font=("Arial", 14), padx=20, pady=20)
